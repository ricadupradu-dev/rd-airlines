...
function haversineNmV30(a,b){ if(!a||!b)return 0; const R=3440.065,rad=Math.PI/180; const dLat=(b[1]-a[1])*rad,dLon=(b[2]-a[2])*rad; const q=Math.sin(dLat/2)**2+Math.cos(a[1]*rad)*Math.cos(b[1]*rad)*Math.sin(dLon/2)**2; return 2*R*Math.asin(Math.min(1,Math.sqrt(q))); }
function cruiseSpeedKtV30(model){ const m=String(model||'').toUpperCase(); if(m.includes('CESSNA 208'))return 145; if(m.includes('ATR 72'))return 255; if(m.includes('E195'))return 445; if(m.includes('A320')||m.includes('A321'))return 450; if(m.includes('A330')||m.includes('A340'))return 470; if(m.includes('A350')||m.includes('A380'))return 480; return 440; }
const RD_SIM_TIME_SCALE_V30=60; // 1 segundo real = aproximadamente 1 minuto de voo simulado.
function routeDurationSecondsV30(x,f){ const a=rdCoordV29(f.origin),b=rdCoordV29(f.dest); const nm=haversineNmV30(a,b); if(!nm)return 180; const cruise=cruiseSpeedKtV30(x?.model); const blockHours=(nm/cruise)+0.70; const simSeconds=Math.round(blockHours*RD_SIM_TIME_SCALE_V30); return Math.max(45,Math.min(1800,simSeconds)); }
function phaseForElapsedV30(elapsed,total){ const taxi=Math.min(10,total*0.08),takeoff=Math.min(4,total*0.04),landing=Math.min(6,total*0.06),descent=Math.min(12,total*0.10); const cruiseEnd=Math.max(taxi+takeoff,total-descent-landing); if(elapsed<taxi)return ['TAXI',elapsed/Math.max(1,taxi)]; if(elapsed<taxi+takeoff)return ['TAKEOFF',(elapsed-taxi)/Math.max(1,takeoff)]; if(elapsed<cruiseEnd)return ['CRUISE',(elapsed-taxi-takeoff)/Math.max(1,cruiseEnd-taxi-takeoff)]; if(elapsed<total-landing)return ['DESCENT',(elapsed-cruiseEnd)/Math.max(1,total-landing-cruiseEnd)]; if(elapsed<total)return ['LANDING',(elapsed-(total-landing))/Math.max(1,landing)]; return ['ARRIVED',1]; }
function routeDurationLabelV30(seconds){ const mins=Math.max(1,Math.round(seconds/60)); if(mins<60)return `${mins} min`; const h=Math.floor(mins/60),m=mins%60; return `${h}h${m?` ${m}min`:''}`; }
